...
it, logs the value and the stack, and records it exactly like any other failure — eighteen
collectors share one process, and one unexpected API response must not stop the other
seventeen, restart after restart. A refresh cut short by shutdown is the opposite case: once
